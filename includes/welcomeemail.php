...
function sm_settings_page_init() {
    add_options_page(__('Messages système', 'csp'), __('Messages système', 'csp'), 'manage_options', 'system_messages', 'system_messages_settings_page');
}
...
function sm_settings_setup() {
    ...
    add_settings_section('sm_settings_general', __('Options générales', 'csp'), 'sm_general_settings_output', 'system_messages');
    add_settings_field('sm_settings[header_from_email]', __('Courriel «De»', 'csp'), 'sm_settings_header_from_email_output', 'system_messages', 'sm_settings_general');
    add_settings_field('sm_settings[header_from_name]', __('Nom «De»', 'csp'), 'sm_settings_header_from_name_output', 'system_messages', 'sm_settings_general');

    add_settings_section('sm_settings_welcome', __('Courriel de bienvenue', 'csp'), 'sm_welcome_settings_output', 'system_messages');
    add_settings_field('sm_settings[user_subject]', __('Sujet', 'csp'), 'sm_settings_user_subject_output', 'system_messages', 'sm_settings_welcome');
    add_settings_field('sm_settings[user_body]', __('Message', 'csp'), 'sm_settings_user_body_output', 'system_messages', 'sm_settings_welcome');
    add_settings_field('sm_settings[header_reply_to]', __('Courriel «Répondre à»', 'csp'), 'sm_settings_header_reply_to_output', 'system_messages', 'sm_settings_welcome');
    add_settings_field('sm_settings[header_additional]', __('Entêtes supplémentaires', 'csp'), 'sm_settings_header_additional_output', 'system_messages', 'sm_settings_welcome');
    ...
    add_settings_field('sm_settings[password_reminder_body]', __('Message', 'csp'), 'sm_settings_password_reminder_body_output', 'system_messages', 'sm_settings_password_reminder');
 ...
 * @param type $passing_through
 * @return type
 */
function system_messages_test($passing_through){
    
    if (isset($_POST['sm_submit_and_test'])){
        global $current_user;
        get_currentuserinfo();
        
        $test = wp_new_user_notification($current_user->ID, null, 'both');
...
function system_messages_settings_page() {
    ...
    ?>
    <div class="wrap">
        <h1><?php _e('Messages système', 'csp'); ?></h1>
        <form method="post" action="options.php">
            ...
            ?>
            <p>
                <?php submit_button(__('Enregistrer les modifications et tester les messages','csp'),'secondary','sm_submit_and_test',false); ?>
                &nbsp;
...
function sm_general_settings_output() {
    //_e('These settings effect all of this plugin and, in some cases, all of your site.', 'csp');
    _e('Ces configurations affectent tous les courriels envoyés par le site.', 'csp');
}
...
function sm_settings_header_from_email_output() {
    ...
    echo '<input type="text" name="sm_settings[header_from_email]" id="sm_setting_header_from_email" value="'.$header_from_email.'"/>'
            . '<p class="description">'
            //. __('Global option change the from email address for all site emails', 'csp')
            . __('Change l’adresse d’origine de tous les courriels du site.', 'csp')
            . '</p>';
}
function sm_settings_header_from_name_output() {
    ...
    echo '<input type="text" name="sm_settings[header_from_name]" id="sm_setting_header_from_name" value="'.$header_from_name.'"/>'
            . '<p class="description">'
            //. __('Global option change the from name for all site emails', 'csp')
            . __('Change le nom d’origine de tous les courriels du site.', 'csp')
            . '</p>';
}
...
function sm_welcome_settings_output() {
    //_e('These settings are for the email sent to the new user on their signup.', 'csp');
    _e('Ces configurations s’appliquent aux courriels envoyés aux nouveaux utilisateurs.', 'csp');
}
function sm_settings_user_subject_output() {
    ...
    echo '<input class="widefat" type="text" name="sm_settings[user_subject]" id="sm_setting_user_subject" value="'.$user_subject.'"/>'
            . '<p class="description">'
            //. __('Subject line for the welcome email sent to the user.', 'csp')
            . __('Sujet du courriel envoyé au nouvel utilisateur.', 'csp')
            . '</p>';
}
...
function sm_settings_user_body_output() {
    ...
    echo '<p class="description">'
            //. __('Body content for the welcome email sent to the user.', 'csp')
            . __('Message du courriel envoyé au nouvel utilisateur.', 'csp')
            .'</p>';
}
function sm_settings_header_reply_to_output() {
    ...
    echo '<input class="widefat" type="text" name="sm_settings[header_reply_to]" id="sm_setting_header_reply_to" value="'.$header_reply_to.'"/>'
            . '<p class="description">'
            . __('Optionnel. Adresse de réponse du courriel envoyé au nouvel utilisateur.', 'csp')
            //. __('Optional Header sent to change the reply to address for new user notification.', 'csp')
            . '</p>';
}
...
function sm_admin_settings_output() {
    //_e('These settings are for the email sent to the admin on a new user signup.', 'csp');
    _e('Ces configurations s’appliquent aux courriels envoyés aux administrateurs lors de l’inscription de nouveaux utilisateurs.', 'csp');
}
function sm_settings_admin_subject_output() {
    ...
    echo '<input class="widefat" type="text" name="sm_settings[admin_subject]" id="sm_setting_admin_subject" value="'.$admin_subject.'"/>'
            . '<p class="description">'
            . __('Sujet du courriel envoyé aux administrateurs sélectionnés.', 'csp')
            //. __('Subject Line for the email sent to the admin user(s)', 'csp')
            . '</p>';
}
...
function sm_settings_admin_body_output() {
    ...
    echo '<p class="description">'
            . __('Message du courriel envoyé aux administrateurs sélectionnés.', 'csp')
            //. __('Body content for the email sent to the admin user(s).', 'csp')
            .'</p>';
}
...
function sm_settings_admin_notify_user_id_output() {
    ...
    if (1==count($admins)){
        ...
        foreach ($admins as $admin){
            echo '<label for="admin_id_'
                    ...
                    . $admin->ID
                    . '"'
                    . checked( in_array($admin->ID, $admin_notify_user_id ) , true)
                    . ' />'
                    . $admin->display_name
                    . '</label>';
        }
    ...
    echo '</p>';
    echo '<p class="description">'
            . __('Sélectionnez les administrateurs qui recevront ce message.', 'csp')
            . '</p>';
}
...
function sm_password_reminder_settings_output() {
    //_e('These settings are for the email sent to the admin on a new user signup.', 'csp');
    _e('Ces configurations s’appliquent aux courriels envoyés pour renouveller les mots de passe.', 'csp');
}
function sm_settings_password_reminder_subject_output() {
    ...
    echo '<input class="widefat" type="text" name="sm_settings[password_reminder_subject]" id="sm_setting_password_reminder_subject" value="'.$password_reminder_subject.'"/>'
            . '<p class="description">'
            . __('Sujet du courriel envoyé à l’utilisateur.', 'csp')
            . '</p>';
}
...
function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
	...
	wp_mail($user->user_email, $user_subject, $user_body, $header_additional);
}
endif;
