 ...
 * @uses CSP_IMAGE_MAX_WIDTH, CSP_IMAGE_MAX_HEIGHT, CSP_IMAGE_QUALITY
 * @see wp_handle_upload_prefilter Filter
 * @return File_ressource
 */
function csp_upload_processor($file) {
    ...
    if (in_array($file['type'], $image_filetypes)) {
        ...
        if (
                (isset($size['width']) && $size['width'] > CSP_IMAGE_MAX_WIDTH )
                ||
                (isset($size['height']) && $size['height'] > CSP_IMAGE_MAX_HEIGHT )
            ){
            $image_editor->set_quality(CSP_IMAGE_QUALITY);
            $image_editor->resize(CSP_IMAGE_MAX_WIDTH, CSP_IMAGE_MAX_HEIGHT,false);
...
add_filter('wp_handle_upload_prefilter', 'csp_upload_processor');
